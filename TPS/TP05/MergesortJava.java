...
class MergesortJava {
    ...
    private static int compare(MergesortJava a, MergesortJava b) {
        if (a.price < b.price) return -1; // menor preço vem primeiro
        if (a.price > b.price) return 1;  // maior preço vem depois
        return Integer.compare(a.id, b.id); // desempate por ID
    }

    private static void merge(MergesortJava[] array, int esq, int meio, int dir, int[] comparisons, int[] movements) {
        int n1 = meio - esq + 1; // tamanho esquerda
        int n2 = dir - meio; // tamanho direita

        MergesortJava[] L = new MergesortJava[n1]; // parte esquerda
        MergesortJava[] R = new MergesortJava[n2]; // parte direita

        for (int i = 0; i < n1; i++)  {
            L[i] = array[esq + i]; // copia esquerda
        }

        for (int j = 0; j < n2; j++) { 
            R[j] = array[meio + 1 + j]; // copia direita
        }

        int i = 0, j = 0, k = esq; // índices de controle

        while (i < n1 && j < n2) {
            comparisons[0]++; // conta comparação
            if (compare(L[i], R[j]) <= 0) array[k++] = L[i++]; // coloca menor
            else array[k++] = R[j++]; // coloca maior
            movements[0]++; // conta movimentação
        }

        while (i < n1) array[k++] = L[i++]; // adiciona restos da esquerda
        while (j < n2) array[k++] = R[j++]; // adiciona restos da direita
    }

    public static void mergeSort(MergesortJava[] array, int esq, int dir, int[] comparisons, int[] movements) {
        if (esq < dir) {
            int meio = (esq + dir) / 2; // acha o meio
            mergeSort(array, esq, meio, comparisons, movements); // ordena esquerda
            mergeSort(array, meio + 1, dir, comparisons, movements); // ordena direita
            merge(array, esq, meio, dir, comparisons, movements); // junta tudo
        }
    }

    public static void showResults(MergesortJava[] games) {
        System.out.println("| 5 preços mais caros |");
        for (int i = games.length - 1; i >= Math.max(games.length - 5, 0); i--)
            System.out.println(games[i]); // mostra do mais caro ao 5º

        System.out.println("| 5 preços mais baratos |");
        for (int i = 0; i < Math.min(5, games.length); i++)
            System.out.println(games[i]); // mostra os 5 mais baratos
    }

    public static void createLog(String matricula, int comparisons, int movements, int time) throws Exception {
        try (PrintWriter log = new PrintWriter(matricula + "_mergesort.txt", "UTF-8")) {
    ...
    private static MergesortJava[] loadGames(String path) throws Exception {
        BufferedReader br = new BufferedReader(new FileReader(path));
        br.readLine(); // ignora cabeçalho
        ArrayList<MergesortJava> list = new ArrayList<>();

        String currentLine;
        while ((currentLine = br.readLine()) != null) {
            MergesortJava g = new MergesortJava();
            g.readLine(currentLine); // lê e converte linha
            list.add(g); // adiciona à lista
        }

        br.close();
        MergesortJava[] games = new MergesortJava[list.size()];
        
        for (int i = 0; i < list.size(); i++) {
            games[i] = list.get(i); // copia lista pra vetor
        }
        
        return games; // retorna todos os jogos
    }

    private static MergesortJava[] selectGamesByIds(int[] ids, MergesortJava[] games) {
        ArrayList<MergesortJava> sel = new ArrayList<>();
        ...
        MergesortJava[] selected = new MergesortJava[sel.size()];
    ...
    public static void read() throws Exception {
        ...
        MergesortJava[] allGames = loadGames("/tmp/games.csv"); // carrega todos os jogos
        MergesortJava[] selected = selectGamesByIds(ids, allGames); // filtra pelos ids
        ...
        if (selected.length > 0)
            mergeSort(selected, 0, selected.length - 1, comparisons, movements); // ordena vetor

        int time = (int) (System.currentTimeMillis() - start); // calcula tempo total

        showResults(selected); // mostra os resultados
        createLog("869624", comparisons[0], movements[0], time); // gera arquivo de log
    }
